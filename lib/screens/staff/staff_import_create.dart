// lib/screens/staff/staff_import_create.dart
import 'package:flutter/material.dart';
import '../../services/api_service.dart';

class StaffImportCreate extends StatefulWidget {
  final int userId;

  const StaffImportCreate({super.key, required this.userId});

  @override
  State<StaffImportCreate> createState() => _StaffImportCreateState();
}

class _StaffImportCreateState extends State<StaffImportCreate> {
  List products = [];
  bool loading = true;
  ...
  Future<void> _saveImportOrder() async {
    if (importItems.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Chưa có sản phẩm')),
      );
      return;
    }

    final ok = await ApiService.addImportOrder(importItems, widget.userId);
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('Tạo phiếu nhập (Nhân viên)')),
      body: Padding(
